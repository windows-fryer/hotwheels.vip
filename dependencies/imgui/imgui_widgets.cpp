...
bool ImGui::Checkbox( const char* label, bool* v, int x_pos )
{
	...
	RenderFrame( check_bb.Min, check_bb.Max, c::get<color>( vars.color_menu_accent ).get_u32( it_filled_frame->second ), true, style.FrameRounding );

	if ( it_filled_frame->second == 1.f )
		RenderFrame( check_bb.Min, check_bb.Max, c::get<color>( vars.color_menu_accent ).get_u32( 0.05f ), true, style.FrameRounding );
...
bool ImGui::BeginCombo( const char* label, const char* preview_value, ImGuiComboFlags flags )
{
	...
	if ( !( flags & ImGuiComboFlags_NoPreview ) )
		window->DrawList->AddRectFilled( frame_bb.Min, ImVec2( value_x2, frame_bb.Max.y ), ImColor( 20 / 255.f, 20 / 255.f, 20 / 255.f, style.Alpha ), style.FrameRounding, ( flags & ImGuiComboFlags_NoArrowButton ) ? ImDrawCornerFlags_All : ImDrawCornerFlags_Left );
	if ( !( flags & ImGuiComboFlags_NoArrowButton ) )
	{
		...
		if ( value_x2 + arrow_size - style.FramePadding.x <= frame_bb.Max.x )
			RenderArrow( window->DrawList, ImVec2( value_x2 + style.FramePadding.y, frame_bb.Min.y + style.FramePadding.y + 2 ), ImColor( 1.f, 1.f, 1.f ), popup_open ? ImGuiDir_Up : ImGuiDir_Down, 0.5f );
	}
	...
	it_filled_frame->second = ImClamp( it_filled_frame->second + ( 2.35f * ImGui::GetIO( ).DeltaTime * ( hovered || g.ActiveId == id || popup_open ? 2.f : -2.f ) ), 0.5f, 1.f );
	it_filled_frame->second *= g.Style.Alpha;

	 PushStyleColor( ImGuiCol_Text, ImVec4( 1.f, 1.f, 1.f, it_filled_frame->second ) );

	if ( preview_value != NULL && !( flags & ImGuiComboFlags_NoPreview ) )
		RenderTextClipped( ImVec2( frame_bb.Min.x + style.FramePadding.y, frame_bb.Min.y + style.FramePadding.y - 1.5f ), ImVec2( value_x2, frame_bb.Max.y - 1.5f ), preview_value, NULL, NULL, ImVec2( 0.0f, 0.0f ) );
...
bool ImGui::SliderScalar( const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, float power )
{
	...
	// modified by qo0
	const ImRect frame_bb( ImVec2( window->DC.CursorPos.x - 1, window->DC.CursorPos.y ) + ImVec2( label_size.x > 0.0f ? style.ItemInnerSpacing.x + h - 1 : 0.0f, label_size.x > 0.0f ? label_size.y : 0.0f ), window->DC.CursorPos + ImVec2( w, ( label_size.x > 0.0f ? label_size.y : 0.0f ) + h ) );
	const ImRect total_bb( window->DC.CursorPos, frame_bb.Max - ImVec2( ( label_size.x > 0.0f ? style.ItemInnerSpacing.x + h : 0.0f ), 0.0f ) );
	...
	it_filled->second = ImClamp( it_filled->second + ( 2.35f * ImGui::GetIO( ).DeltaTime * ( hovered || g.ActiveId == id ? 2.f : -2.f ) ), 0.5f, 1.f );
	it_filled->second *= g.Style.Alpha;

	// Render grab
	if ( grab_bb.Max.x > grab_bb.Min.x )
	{
		window->DrawList->AddRectFilled( grab_bb.Min, grab_bb.Max, c::get<color>( vars.color_menu_accent ).get_u32( ), style.FrameRounding );
	...
	char value_buf[ 64 ];
	const char* value_buf_end = value_buf + DataTypeFormatString( value_buf, IM_ARRAYSIZE( value_buf ), data_type, p_data, format );
	RenderText( ImVec2( frame_bb.Max.x + 1.f - CalcTextSize( value_buf ).x - 1.0f, total_bb.Min.y + style.FramePadding.y - 1 ), value_buf, value_buf_end );
...
bool ImGui::InputTextEx( const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data )
{
	...
	if ( !is_multiline )
	{
		// modified by qo0
		RenderNavHighlight( frame_bb, id );
		window->DrawList->AddRectFilled( frame_bb.Min, frame_bb.Max, ImColor( 20 / 255.f, 20 / 255.f, 20 / 255.f, g.Style.Alpha ), style.FrameRounding );
	...
	it_filled_frame->second = ImClamp( it_filled_frame->second + ( 2.35f * ImGui::GetIO( ).DeltaTime * ( hovered || g.ActiveId == id ? 2.f : -2.f ) ), 0.5f, 1.f );
	it_filled_frame->second *= g.Style.Alpha;
...
static void ColorPickerOptionsPopup2( ImGuiColorEditFlags flags, float* ref_col )
{
	...
	if ( allow_opt_picker ) {
		...
		ImVec2 picker_size( g.FontSize * 8, ImMax( g.FontSize * 8 - ( ( g.FontSize + g.Style.FramePadding.y * 2.0f ) + g.Style.ItemInnerSpacing.x ), 1.0f ) ); // FIXME: Picker size copied from main picker function
...
bool ImGui::ColorButton( const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size )
{
	...
	window->DrawList->AddRect( bb.Min, bb.Max, ImColor(20 / 255.f, 20 / 255.f, 20 / 255.f, g.Style.Alpha), rounding ); // Color button are often in need of some sort of border
...
bool ImGui::Selectable( const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg )
{
	...
	it_filled_frame->second = ImClamp( it_filled_frame->second + ( 2.35f * ImGui::GetIO( ).DeltaTime * ( selected || hovered ? 2.f : -2.f ) ), 0.5f, 1.f );
	it_filled_frame->second *= g.Style.Alpha;
