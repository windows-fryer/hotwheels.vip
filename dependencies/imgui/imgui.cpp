...
bool ImGui::Keybind( const char* str_id, int* current_key, int* key_style, int sameline ) {
	...
	opened_frame->second = ImClamp( opened_frame->second + ( 2.35f * ImGui::GetIO( ).DeltaTime * ( IsPopupOpen( id ) ? 2.f : -2.f ) ), 0.0f, 1.f );
	...
	if ( g.ActiveId == id ) {
		...
		if ( key_style ) {
			...
			PushStyleColor( ImGuiCol_PopupBg, ImVec4( 20 / 255.f, 20 / 255.f, 20 / 255.f, opened_frame->second ) );
			PushStyleColor( ImGuiCol_WindowBg, ImVec4( 20 / 255.f, 20 / 255.f, 20 / 255.f, opened_frame->second ) );

			if ( style_requested && !popup_open )
				OpenPopupEx( id );

			if ( popup_open ) {
				SetNextWindowSize( ImVec2( 45, CalcMaxPopupHeightFromItemCount( 3 ) ) );
				...
				if ( Selectable( "always", *key_style == 0 ), ImGuiSelectableFlags_DontClosePopups )
					*key_style = 0;

				if ( Selectable( "hold", *key_style == 1 ), ImGuiSelectableFlags_DontClosePopups )
					*key_style = 1;

				if ( Selectable( "toggle", *key_style == 2 ), ImGuiSelectableFlags_DontClosePopups )
					*key_style = 2;
