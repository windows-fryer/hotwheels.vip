...
namespace detail
{
	...
	FILE* get_file( std::basic_filebuf< Char >& )
	{
		return nullptr;
	}
	...
	template< typename T >
	struct ms_filebuf< T, decltype( T::_Myfile, 0 ) > {
		using type = T;
	};
	...
	template class filebuf_access< filebuf_access_tag, decltype( &filebuf_type::_Myfile ), &filebuf_type::_Myfile >;
	...
	void write_buffer( std::basic_ostream< Char >& os, buffer< Char >& buf )
	{
		if ( const_check( FMT_MSC_VERSION ) ) {
			...
			if ( filebuf && write( *filebuf, { buf.data( ), buf.size( ) } ) )
				return;
		}
