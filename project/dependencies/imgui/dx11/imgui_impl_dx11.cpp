...
//  2019-07-21: DirectX11: Backup, clear and restore Geometry Shader is any is bound when calling ImGui_ImplDX10_RenderDrawData. Clearing
//  Hull/Domain/Compute shaders without backup/restore. 2019-05-29: DirectX11: Added support for large mesh (64K+ vertices), enable
//  ImGuiBackendFlags_RendererHasVtxOffset flag. 2019-04-30: DirectX11: Added support for special ImDrawCallback_ResetRenderState callback to reset
//  render state. 2018-12-03: Misc: Added #pragma comment statement to automatically link with d3dcompiler.lib when using D3DCompile(). 2018-11-30:
//  Misc: Setting up io.BackendRendererName so it can be displayed in the About Window. 2018-08-01: DirectX11: Querying for IDXGIFactory instead of
//  IDXGIFactory1 to increase compatibility. 2018-07-13: DirectX11: Fixed unreleased resources in Init and Shutdown functions. 2018-06-08: Misc:
//  Extracted imgui_impl_dx11.cpp/.h away from the old combined DX11+Win32 example. 2018-06-08: DirectX11: Use draw_data->DisplayPos and
//  draw_data->DisplaySize to setup projection matrix and clipping rectangle. 2018-02-16: Misc: Obsoleted the io.RenderDrawListsFn callback and
//  exposed ImGui_ImplDX11_RenderDrawData() in the .h file so you can call it yourself. 2018-02-06: Misc: Removed call to ImGui::Shutdown() which is
//  not available from 1.60 WIP, user needs to call CreateContext/DestroyContext themselves. 2016-05-07: DirectX11: Disabling depth-write.
...
bool ImGui_ImplDX11_CreateDeviceObjects( )
{
	...
	// Create the vertex shader
	{
		...
		if ( g_pVertexShaderBlob == NULL ) // NB: Pass ID3D10Blob* pErrorBlob to D3DCompile() to get error showing in (const
		                                   // char*)pErrorBlob->GetBufferPointer(). Make sure to Release() the blob!
			return false;
		if ( g_pd3dDevice->CreateVertexShader( ( DWORD* )g_pVertexShaderBlob->GetBufferPointer( ), g_pVertexShaderBlob->GetBufferSize( ), NULL,
		...
		if ( g_pd3dDevice->CreateInputLayout( local_layout, 3, g_pVertexShaderBlob->GetBufferPointer( ), g_pVertexShaderBlob->GetBufferSize( ),
		                                      &g_pInputLayout ) != S_OK )
			return false;
	...
	// Create the pixel shader
	{
		...
		if ( g_pPixelShaderBlob == NULL ) // NB: Pass ID3D10Blob* pErrorBlob to D3DCompile() to get error showing in (const
		                                  // char*)pErrorBlob->GetBufferPointer(). Make sure to Release() the blob!
			return false;
		if ( g_pd3dDevice->CreatePixelShader( ( DWORD* )g_pPixelShaderBlob->GetBufferPointer( ), g_pPixelShaderBlob->GetBufferSize( ), NULL,
		                                      &g_pPixelShader ) != S_OK )
			return false;
	}
