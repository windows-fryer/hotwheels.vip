...
#ifndef HOTWHEELS_CSGO_EXTERNAL_MATRIX_HPP
#define HOTWHEELS_CSGO_EXTERNAL_MATRIX_HPP
	...
	public:
		...
		vector to_angle( )
		{
			...
			if ( xyDist > 0.001f ) {
				...
				angles.y = ( ( float )( ( atan2f( forward[ 1 ], forward[ 0 ] ) ) ) * ( float )( 180.0f / ( float )( 3.14159265358979323846f ) ) );

				// (pitch)	x = ATAN( -forward.z, sqrt(forward.x*forward.x+forward.y*forward.y) );
				angles.x = ( ( float )( ( atan2f( -forward[ 2 ], xyDist ) ) ) *
				             ( float )( 180.0f / ( float )( 3.14159265358979323846f ) ) ); // RAD2DEG( atan2f( -forward[ 2 ], xyDist ) );

				// (roll)	z = ATAN( left.z, up.z );
				angles.z = ( ( float )( ( atan2f( left[ 2 ], up[ 2 ] ) ) ) * ( float )( 180.0f / ( float )( 3.14159265358979323846f ) ) );
				// RAD2DEG( atan2f( left[ 2 ], up[ 2 ] ) );
			} else // forward is mostly Z, gimbal lock-
			{
				// (yaw)	y = ATAN( -left.x, left.y );			-- forward is mostly z, so use right for yaw
				angles.y = ( ( float )( ( atan2f( -left[ 0 ], left[ 1 ] ) ) ) * ( float )( 180.0f / ( float )( 3.14159265358979323846f ) ) );

				// RAD2DEG( atan2f( -left[ 0 ], left[ 1 ] ) );

				// (pitch)	x = ATAN( -forward.z, sqrt(forward.x*forward.x+forward.y*forward.y) );
				angles.x = ( ( float )( ( atan2f( -forward[ 2 ], xyDist ) ) ) * ( float )( 180.0f / ( float )( 3.14159265358979323846f ) ) );
...
#endif // HOTWHEELS_CSGO_EXTERNAL_MATRIX_HPP
