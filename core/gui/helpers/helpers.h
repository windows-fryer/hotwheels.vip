...
namespace gui::helpers
{
	inline auto render_tab = [ ]( const char* tab_title, int tab_number )
	{
		...
		it_filled->second = ImClamp( it_filled->second + ( 2.35f * ImGui::GetIO( ).DeltaTime * ( gui::g_data.m_tab == tab_number ? 2.f : -2.f ) ), 0.5f, 1.f );
		it_filled->second *= c::get<color>( vars.color_menu_accent ).base<color_a>( );

		ImGui::TextColored( gui::g_data.m_tab == tab_number ? c::get<color>( vars.color_menu_accent ).get_imvec4( it_filled->second ) : ImVec4( 1.f, 1.f, 1.f, 0.5f ), tab_title ); if ( ImGui::IsItemClicked( ) ) gui::g_data.m_tab = tab_number;  ImGui::SameLine( );
	};
	...
	inline auto render_color_picker = [ ]( const char* label, color* col, bool alpha, int sameline, bool text )
	{
		...
		if ( ImGui::ColorEdit4( std::string( _( "##" ) ).append( label ).c_str( ), &output_color.x, alpha_slider_flag | ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_NoTooltip ) )
		{
	...
	inline auto render_keybind = [ ]( const char* label, int* key, int* keystyle, int sameline, bool text )
	{
		...
		ImGui::Keybind( std::string( _( "##" ) ).append( label ).c_str( ), key, keystyle, sameline );
	};
	...
	inline auto render_combo = [ ]( const char* label, int* current_item, const char* items_separated_by_zeros, int pos_x )
	{
		...
		opened_frame->second = ImClamp( opened_frame->second + ( 2.35f * ImGui::GetIO( ).DeltaTime * ( ImGui::IsPopupOpen( label ) ? 2.f : -2.f ) ), 0.0f, 1.f );
	...
	inline auto render_multi_combo = [ ]( const char* label, bool* values, const std::string_view* items, int items_count, int pos_x ) -> bool
	{
		...
		if ( buffer.empty( ) )
			buffer.assign( _( "none" ) );
		else
		{
			const char* szWrapPosition = g.Font->CalcWordWrapPositionA( ImGui::GetCurrentWindow( )->FontWindowScale, &buffer[ 0 ], buffer.data( ) + buffer.length( ), active_width - style.FramePadding.x * 2.0f );
		...
		opened_frame->second = ImClamp( opened_frame->second + ( 2.35f * ImGui::GetIO( ).DeltaTime * ( ImGui::IsPopupOpen( id ) ? 2.f : -2.f ) ), 0.0f, 1.f );
		opened_frame->second *= ImGui::GetStyle( ).Alpha;
	...
	inline auto render_listbox = [ ]( const char* label, int* current_item, std::function<const char* ( int )> lambda, int items_count, int height_in_items )
	{
		auto do_list_box = [ & ]( )
		{
			return ImGui::ListBox( label, current_item, [ ]( void* data, int index, const char** out_text )
				{
